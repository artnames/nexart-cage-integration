...
Nine curated machine-readable test summaries are retained under [`results/`](./results/), alongside the repository artifact manifest. They describe the original acceptance work and are not raw CI logs. See [`results/README.md`](./results/README.md) for provenance, reproduction status and the precise claim boundary. The artifact manifest binds the current repository files.
...
Public and local read-only validation requires no credential:

```bash
npm run test:public
```

This runs the tamper fixture, privacy/publication, independent-cryptography and local minimal-fixture checks. It does not recreate the authenticated historical phases.
